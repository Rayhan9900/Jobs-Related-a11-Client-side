...
function BidRequests() {
    ...
    const { data: bidRequests = [], refetch } = useQuery({
        queryKey: ['bids'],
        queryFn: async () => {
            const res = await axios.get(`https://jobs-related-server-ass-11.vercel.app/bids/${user?.email}`);
            return res.data;
        }
    })

    const handleAccept = (id) => {
        axios.patch(`https://jobs-related-server-ass-11.vercel.app/bid-requests/accept/${id}`)
        .then(res => {
            refetch()
        })
    };
    const handleReject = (id) => {
        axios.patch(`https://jobs-related-server-ass-11.vercel.app/bid-requests/reject/${id}`)
        .then(res => {
            refetch()
        })
    };
